...
QoSmate and Speedify are off until enabled in Mega. Guarded per-modem recovery is enabled by default and can be disabled per slot. No build forces a cell, PCC, band set, carrier MBN profile or undocumented EFS setting. A cell lock can prevent additional cells from attaching on this modem family, so public defaults stay unlocked and carrier-neutral.

Advanced opens on **5G & Network Mode**. **Automatic preferred** is the default and delegates SA/NSA selection entirely to the modem/network without test downloads. **Automatic adaptive** is an explicit opt-in comparison with idle/backup guards, rollback and a data cap. Manual **NSA only** and **SA only** are preserved. No choice guesses from the SIM operator or rewrites band masks. Opening the page is read-only. See [adaptive behavior and validation](adaptive-5g-and-failback.md).

For the reported 30–32 Mbps cases, the screenshots show SA while the faster router shows LTE/NSA aggregation. This supports comparing deployments, but does not prove SA caused the difference or establish that Modem 1 had a working data session. Adaptive therefore checks real IPv4 reachability and samples the actual selected modem, not the router default route. Manual comparisons should still use the same SIM, location, wired client, server and time window. Keep the other modem as failover; don’t treat radio registration as proof of Internet access.
