...
(async () => {
  ...
  try {
    ...
    assert.match(await page.locator('#zma-features').textContent(), /Automatic preferred[\s\S]*Automatic adaptive[\s\S]*NSA only[\s\S]*SA only/);
    assert.match(await page.locator('#zma-features').textContent(), /300 MB per modem per 24 hours/);
