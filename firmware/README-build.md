...
- Per-modem procd instances replace the global restart/auto-enable repair loops. Each enabled instance waits for its physical slot's exact USB net device and AT port before launching the dialer, so unequal boot enumeration cannot permanently skip Modem 1. The optional watchdog can act only on the selected modem and only when its actions are explicitly enabled. Automatic post-flash modem factory resets are retired.
