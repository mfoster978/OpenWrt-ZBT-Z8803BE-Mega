...
- Per-modem procd instances replace the global restart/auto-enable repair loops. Each enabled instance waits for its physical slot's exact USB net device and AT port, serializes the two initial setup phases, and explicitly relaunches an exited dialer, so unequal boot enumeration or a QMI process loss cannot permanently skip Modem 1. Recovery can act only on the selected modem. Automatic post-flash modem factory resets are retired.
